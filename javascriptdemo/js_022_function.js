...
//함수(function) : 특정 기능을 하는 구문을 독립된 부품으로 만들어 재사용하고자 할때 사용하는
//문법이다.
...
//클로저(closure)
/*
 1. 중첩함수는 outer(외부)함수가 끝나면 외부에서 outer(외부)함수의 자원을 사욜할 수 없다.
 2. 클로저는 outer(외부)함수보다 중첩함수가 더 오래 유지되는 경우 중첩함수는 이미
 생명주기가 종료한 외부함수의 변수를 참조할 수 있다. 이러한 중첩함수가 클로저(closure)이다.
 3. 클로저는 변수가 의도치 않게 변경되지 않도록 안전하세 은닉(information hiding)하고 
...
//(1) 기본 파라미터(default parametter) : ES6-2015
...
//(2) 나머지 피라미터(Rest Panameter) : ES6-2015
...
// spread operator를 고정 파라미터와 같이 사용할깨는 맨 마지막에 한번만 사용한다.
